...
class StatisticalAnalyzer:
    ...
    def analyze_football(self, team1: str, team2: str) -> Dict[str, float]:
        ...
        # Expected score from Win Rate
        win_rate_diff = s1["win_rate"] - s2["win_rate"]
        e1 = 0.5 + (win_rate_diff * 0.5)
        e2 = 1.0 - e1
        ...
        # Combine Elo, momentum, and goal scoring trends (allow slight negative penalties)
        score1 = max(0.01, e1 * 0.6 + max(trend1, -0.15) * 0.2 + max(goal_trend1, -0.15) * 0.2)
        score2 = max(0.01, e2 * 0.6 + max(trend2, -0.15) * 0.2 + max(goal_trend2, -0.15) * 0.2)
    ...
    def analyze_cricket(self, team1: str, team2: str) -> Dict[str, float]:
        ...
        score1 = max(0.01, e1 * 0.7 + max(trend1, -0.1) * 0.15 + rt_norm1 * 0.15)
        score2 = max(0.01, e2 * 0.7 + max(trend2, -0.1) * 0.15 + rt_norm2 * 0.15)

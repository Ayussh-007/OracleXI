...
class AnalyticsPage(tk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.current_category = "International"
    ...
    def _on_category_change(self, event: tk.Event = None) -> None:
    ...
    def _update_team_list(self) -> None:
        """Update team dropdown for selected sport and category."""
        if self.current_sport == "Football":
            if self.current_category == "Clubs":
                teams = FOOTBALL_CLUBS
            else:
                teams = FOOTBALL_INTERNATIONAL
        else:
            if self.current_category == "T20 Franchise":
                teams = CRICKET_IPL_TEAMS
            else:
                teams = CRICKET_INTERNATIONAL
    ...
    def _render_charts(self) -> None:
        ...
        # Just one simple, clean chart frame as requested
    ...
    def _chart_win_draw_loss(self, parent: tk.Widget, side: str) -> None:
        ...
        selected = self.team_var.get()
        if selected and selected != "All Teams":
            teams = [selected]
        else:
        ...
        for team in teams:
            stats = self.engine.data_engine.get_football_team_stats(team, category=self.current_category)
    ...
    def _chart_avg_goals(self, parent: tk.Widget, side: str) -> None:
        ...
        selected = self.team_var.get()
        all_teams = self.engine.data_engine.get_football_teams()
        if selected and selected != "All Teams":
            teams = [selected] + [t for t in all_teams if t != selected][:4]
        else:
            teams = all_teams[:5]
    ...
    def _chart_team_performance(self, parent: tk.Widget, side: str) -> None:
        ...
        try:
            ...
            tk.Label(card.inner, text="Chart unavailable", bg=COLORS["bg_secondary"], fg=COLORS["text_secondary"]).pack(expand=True)
    ...
    def _chart_momentum_trend(self, parent: tk.Widget, side: str) -> None:
        ...
        try:
            ...
            for i, team in enumerate(teams):
                rolling = self.engine.data_engine.get_football_rolling_stats(team, category=self.current_category)
    ...
    def _chart_cricket_win_loss(self, parent: tk.Widget, side: str) -> None:
        """Cricket win/loss pie chart for the selected team."""
        card = create_card(parent, padding=10)
        card.pack(side=side, fill="both", expand=True, padx=(0, 6 if side == "left" else 0))
        try:
            fig = Figure(figsize=(5, 3.5), dpi=100)
            ...
            selected = self.team_var.get()
            team = selected if selected and selected != "All Teams" else self.engine.data_engine.get_cricket_teams()[0]
            
            stats = self.engine.data_engine.get_cricket_team_stats(team, category=self.current_category)
            ...
            if total > 0:
                ...
                ax.pie(sizes, labels=labels, colors=colors, startangle=90,
                       textprops={"color": CHART_TEXT, "fontsize": 9}, wedgeprops={'alpha': 0.85})
                ax.set_title(f"Win Distribution ({team})", fontsize=11, color=CHART_TEXT, pad=10)
            else:
                ax.pie([1], labels=["No Data"], colors=[CHART_COLORS[0]], startangle=90)
                ax.set_title(f"Win Distribution ({team})", fontsize=11, color=CHART_TEXT, pad=10)
    ...
    def _chart_avg_runs(self, parent: tk.Widget, side: str) -> None:
        ...
        try:
            ...
            selected = self.team_var.get()
            all_teams = self.engine.data_engine.get_cricket_teams()
            if selected and selected != "All Teams":
                teams = [selected] + [t for t in all_teams if t != selected][:4]
            else:
                teams = all_teams[:5]
    ...
    def _chart_cricket_momentum(self, parent: tk.Widget, side: str) -> None:
        ...
        try:
            ...
            for i, team in enumerate(teams):
                rolling = self.engine.data_engine.get_cricket_rolling_stats(team, category=self.current_category)
                if not rolling.empty and "rolling_win_rate" in rolling.columns:
                    values = rolling["rolling_win_rate"].values[-30:]
